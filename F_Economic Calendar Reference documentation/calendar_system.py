...
try:
    ...
    class MockPandas:
        ...
        def read_csv(file_path):
            return BasicDataFrame.read_csv(file_path)
...
try:
    ...
    print("aiofiles not available - using synchronous file operations")
    aiofiles = None

try:
    import aiohttp
except ImportError:
    print("aiohttp not available - web features disabled")
    aiohttp = None
...
try:
    ...
    print("pydantic not available - using basic data validation")
    class BaseModel:
...
try:
    ...
    print("FastAPI/uvicorn not available - web interface disabled")
    uvicorn = None
    FastAPI = None
...
try:
    ...
    print("websockets not available - real-time updates disabled")
    websockets = None
...
# === Strategy ID config & generator (calendar strategies) ===
try:
...
class StrategyIdConfig(BaseModel):
    base: int = 300
    country_codes: Dict[str, int] = {
        "USD": 1, "EUR": 2, "GBP": 3, "JPY": 4, "CAD": 5, "AUD": 6, "NZD": 7, "CHF": 8
    }
    impact_codes: Dict[str, int] = {"Low": 1, "Medium": 2, "High": 3}
    param_set_by_impact: Dict[str, int] = {"Low": 1, "Medium": 2, "High": 4}
    overrides: List[Dict[str, Any]] = []

class StrategyIdGenerator:
    def __init__(self, cfg):
        self.cfg = cfg
        self._sid_cfg = getattr(cfg, "strategy_id", StrategyIdConfig())
        if not hasattr(cfg, "strategy_id"):
            try:
                setattr(cfg, "strategy_id", self._sid_cfg)
            except Exception:
                pass

    def _cc(self, country: str) -> int:
        ccmap = getattr(self._sid_cfg, "country_codes", {}) or {}
        return int(ccmap.get((country or "").upper(), 9))

    def _ic(self, impact: str) -> int:
        imap = getattr(self._sid_cfg, "impact_codes", {}) or {}
        norm = (impact or "").strip().capitalize()
        return int(imap.get(norm, 1))

    def _formula(self, country: str, impact: str) -> int:
        base = int(getattr(self._sid_cfg, "base", 300))
        return int(base + 10 * self._cc(country) + self._ic(impact))

    def _param_set_from_impact(self, impact: str) -> int:
        imap = getattr(self._sid_cfg, "param_set_by_impact", {}) or {}
        norm = (impact or "").strip().capitalize()
        return int(imap.get(norm, 1))

    def generate(self, event) -> tuple[int, int]:
        title = getattr(event, "title", "") or ""
        overrides = getattr(self._sid_cfg, "overrides", []) or []
        for ov in overrides:
            pat = ov.get("pattern")
            if pat and re.search(pat, title, flags=re.I):
                sid = int(ov.get("strategy_id") or self._formula(getattr(event, "country", ""), getattr(event, "impact", "")))
                pid = int(ov.get("param_set_id") or self._param_set_from_impact(getattr(event, "impact", "")))
                return sid, pid
        return self._formula(getattr(event, "country", ""), getattr(event, "impact", "")), self._param_set_from_impact(getattr(event, "impact", ""))
...
# ============================================================================
    strategy_id: StrategyIdConfig = StrategyIdConfig()
...
class CalendarImportEngine:
    ...
    def __init__(self, config: SystemConfig, db_manager: DatabaseManager):
        self.config = config
        try:
            ensure_strategy_id_defaults(self.config)
        except Exception:
            pass
        self.db = db_manager
...
class CalendarSystem:
            ...
            with open(config_path, 'r') as f:
                if config_path.endswith('.yaml') or config_path.endswith('.yml'):
                    if yaml:
                        ...
                        self.logger.warning("YAML not available, falling back to JSON")
                        return SystemConfig()
                else:
    ...
    async def start_system(self):
        ...
        if self.scheduler and CronTrigger:
            ...
            self.logger.warning("Scheduler not available - running in manual mode")
...
if FastAPI:
    ...
    try:
        ...
        pass  # Static files directory doesn't exist
else:
    app = None
...
async def main():
    ...
    config_file = Path("config.json" if not yaml else "config.yaml")
    ...
    # Initialize system directly if no web server
    if not uvicorn or not app:
        print("Starting calendar system in console mode...")
        ...
        # Start web server
        print("Starting calendar system with web interface...")
...
def ensure_strategy_id_defaults(config) -> None:
    """Ensure `strategy_id` block exists on the config object with sensible defaults.
    If a YAML config writer exists in the app, it will persist on next save.
    """
    try:
        _ = config.strategy_id  # may raise
    except Exception:
        try:
            config.strategy_id = StrategyIdConfig()
        except Exception:
            pass
